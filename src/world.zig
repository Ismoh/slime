...
pub fn World(comptime Bundle: type) type {
    ...
    return struct {
        ...
        pub fn spawn(self: *Self, comptime types: []const type, values: anytype) !Entity {
            ...
            const row = try arch.appendRow(self.allocator, e, Info, sig, types, values);
        ...
        pub fn addComponent(self: *Self, e: Entity, comptime T: type, value: T) !void {
            ...
            const new_row = try dst.pushBlankRow(self.allocator, e, Info, new_sig);
        ...
        /// Iterate entities whose archetype contains every component in `with`.
        pub fn query(self: *Self, comptime with: []const type) QueryIter {
            return .{
                .world = self,
                .arch_index = 0,
                .row = 0,
                .required_mask = Info.maskMany(with),
            };
        }
        ...
        /// Contiguous row range within one archetype (SoA-friendly). `chunk_size` must be > 0.
        ...
        /// Dense `[]T` view into a column when `stride == @sizeOf(T)` (typical for packed components).
        ...
        /// Clears all archetypes and entities (frees storage). Allocator is retained.
        ...
        /// Spawn one entity from a prefab payload (`data` matches snapshot column layout for a single row).
        pub fn spawnPrefab(self: *Self, prefab: prefab_mod.PrefabRef) !Entity {
            const sig = prefab.signature;
            const e = try self.allocEntity();
            const arch_id = try self.ensureArchetype(sig);
            const arch = &self.archetypes.items[arch_id];
            const row_u = try arch.pushBlankRow(self.allocator, e, Info, sig);
            ...
            const r = fbs.reader();
            try fillPrefabRow(self, arch, row, sig, r);
            return e;
        }

        fn fillPrefabRow(self: *Self, arch: *Archetype, row: u32, sig: u64, reader: anytype) !void {
            inline for (0..Info.count) |i| {
                const cid: u32 = @intCast(i);
                ...
                if ((sig & bit) != 0) {
                    ...
                    if (comptime serialize.hasCustomSerialization(CT)) {
                        const plen = try readU32(reader);
                        const buf = try self.allocator.alloc(u8, plen);
                        defer self.allocator.free(buf);
                        try reader.readNoEof(buf);
                        var payload_fbs = std.io.fixedBufferStream(buf);
                        const val = try serialize.deserializeComponent(CT, payload_fbs.reader());
                        const dst = col.rowPtr(@intCast(row))[0..@sizeOf(CT)];
                        @memcpy(dst, std.mem.asBytes(&val));
                    } else {
                        const total = col.stride;
                        try reader.readNoEof(col.data[@as(usize, @intCast(row)) * col.stride ..][0..total]);
                    }
                }
            }
        }

        /// Binary snapshot: magic, version, slot capacity, archetype blobs (entities + columns).
        pub fn writeSnapshot(self: *Self, writer: anytype) !void {
            const snapshot_magic: u32 = 0x4D4C4953; // "SLIM" little-endian
            ...
            for (self.archetypes.items) |*arch| {
                ...
                inline for (0..Info.count) |i| {
                    const cid: u32 = @intCast(i);
                    const bit = @as(u64, 1) << @intCast(cid);
                    if ((arch.signature & bit) != 0) {
                        const CT = Info.typeAt(cid);
                        const col = arch.getColumn(cid).?;
                        if (comptime serialize.hasCustomSerialization(CT)) {
                            for (0..rows) |r| {
                                const bytes = col.rowPtr(r)[0..Info.elementSize(cid)];
                                const val = std.mem.bytesToValue(CT, bytes[0..@sizeOf(CT)]);
                                const owned = try serialize.serializeComponentToSlice(CT, val, self.allocator);
                                defer self.allocator.free(owned);
                                try writeU32(writer, @intCast(owned.len));
                                try writer.writeAll(owned);
                            }
                        } else {
                            const total = col.stride * rows;
        ...
        pub fn readSnapshot(self: *Self, reader: anytype) !void {
            ...
            while (aidx < arch_count) : (aidx += 1) {
                ...
                while (r < rows) : (r += 1) {
                    ...
                    const row = try arch.pushBlankRow(self.allocator, e, Info, sig);
                    self.entities.items[e.index] = .{
                        .generation = e.generation,
                        .alive = true,
                        .archetype = aid,
                    ...
                    if ((sig & bit) != 0) {
                        ...
                        if (comptime serialize.hasCustomSerialization(CT)) {
                            ...
                            while (rr < rows) : (rr += 1) {
                                ...
                                const val = try serialize.deserializeComponent(CT, fbs.reader());
                                const dst = col.rowPtr(@intCast(rr))[0..@sizeOf(CT)];
                                @memcpy(dst, std.mem.asBytes(&val));
                            }
                        } else {
                            const total = col.stride * rows;
